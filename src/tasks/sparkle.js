// (C)opyright 2020-10-22 devmastery
...
export function sparkle({
  ...
  site.xml(path, (ctx) => {
    ...
    ctx.body = (
      <rss
        xmlns__sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle"
        xmlns__dc="http://purl.org/dc/elements/1.1/"
        version="2.0"
      >
        <channel>
          <title>{title}</title>
          <link>{publicURL || url}</link>
          ...
          <item>
            ...
            <sparkle__releaseNotesLink>
              {site.baseURL}
              {feednotesURL}
            </sparkle__releaseNotesLink>
            <pubDate>{r.date?.toGMTString()}</pubDate>
            <enclosure
              url={`${site.baseURL}${r.url}`}
